...
class TestUsers(base.BaseIamResourceTest):
    ...
    def test_captcha(
        ...
        url = urljoin(
            user_api.base_url,
            f"iam/users/{default_client_uuid}/actions/get_captcha",
        )
        ...
        iam_controllers.CONF[iam_controllers.DOMAIN].captcha_required = (
            required
        )

        if mock_verify:
            altcha.verify_solution = mock.MagicMock(return_value=(True, None))
